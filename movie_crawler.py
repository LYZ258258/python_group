...
MOVIE_INFO_PATH = os.path.join(BASE_DIR, "data")
CONFIG_PATH = os.path.join(BASE_DIR, "config.json")

# 自动创建data文件夹保存数据
os.makedirs(os.path.dirname(MOVIE_INFO_PATH), exist_ok=True)
...
class DoubanMovieCrawler(EnhancedCrawler):
    ...
    def parse_movie(self, html):
        ...
        return {
            "movie_id": MOVIE_ID,
            "movie_title": self._safe_extract(soup, 'h1 span[property="v:itemreviewed"]'),
            "movie_year": self._extract_year(soup),
            "movie_rating": self._extract_rating(soup),
...
class DoubanCommentCrawler(EnhancedCrawler):
        ...
        for item in soup.select('div.comment-item'):
            try:
                comment = {
                    ...
                    'comment_cid': item.get('data-cid', ''),
                    'comment_content': item.select_one('span.short').text.strip(),
                    'comment_rating': self._extract_rating(item),
                    'comment_timestamp': item.select_one('span.comment-time')['title'],
    ...
    def _save_comments(self, comments):
        ...
        with self.file_lock:
            try:
                movie_info["comment_list"].extend(comments)
...
if __name__ == "__main__":
    ...
    path = os.path.join(MOVIE_INFO_PATH, file_name)
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(movie_info, f, ensure_ascii=False, indent=2)
        logging.info("电影信息已保存至: %s", path)
